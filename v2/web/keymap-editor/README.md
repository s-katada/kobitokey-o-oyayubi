...
DXF はトッププレートの外形なので、そのままだとネジ座の半円の切り欠きが輪郭に出てしまいます。
組み上げた状態ではその外側をサイドプレートが覆うため見えません。そこで**円弧状の小さな凹み
だけ**を埋めています（4 セグメント以上・9mm 未満・18mm² 未満）。実際の段差は 2〜3 本の直線
なので、大きさではなく形で区別され、列スタッガーもヒンジ切り欠きも残ります。

この位置合わせが将来ずれないよう、テストで固定してあります（全メインキーがキャップ四隅まで
プレート多角形の内側にあること／親指プレート外形の 15mm 切り欠きが該当キー中心と一致する
こと）。
